...
2. **Branches** on Cloud vs On-Premise
3. **Calls an AI node** to generate an implementation plan + architecture diagram
4. **Creates a request entity** in Port with the AI output
5. **Branches** on environment — Production goes to Slack for approval, everything else auto-approves
6. **Opens a PR** (non-production) and creates the cloud resource entity
7. **Notifies Slack** with links to the plan, architecture, and PR
