...
- Tell attendees **Slack nodes may fail** until secrets exist — catalog + form + most steps still land. **`fetch_service_context`** calls **`api.getport.io`** and does not need manual Port API credentials in workflow secrets.
